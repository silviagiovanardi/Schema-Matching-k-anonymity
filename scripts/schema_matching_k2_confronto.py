...
def schema_matching_k2_confronto():
    file_testuale = "data/omop_k2_final.csv"
    file_semantico = "data/omop_k2_semantic.csv"

    # escrizioni OMOP-CDM arricchite con esempi clinici
    omop_schema = {
    ...
    cols_testuale, texts_testuale = preprocess_dataset(file_testuale, "k2_classico")
    cols_semantico, texts_semantico = preprocess_dataset(file_semantico, "k2_semantico")
    ...
    df_testuale = match_columns(vec_testuale, cols_testuale, "k2_classico")
    df_semantico = match_columns(vec_semantico, cols_semantico, "k2_semantico")

    df_finale = pd.concat([df_testuale, df_semantico], ignore_index=True)
    df_finale.to_csv("data/schema_matching_k2_confronto.csv", index=False)
    print("Matching comparativo salvato in schema_matching_k2_confronto.csv")
